...
public class FlyEnemyController : MonoBehaviour,IEnemyBace,IHitPointer,IPoolSeter,IEnemyFly
{
    #region 変数  
    //ステータス変数
    ...
    //飛行エネミーの移動先取得プロパティ
    public Vector2 GetFlyPos => SetRondomPos();
    ...
    //プレイヤーに近寄れるランダム位置取得
    private Vector2 SetRondomPos()
    {
        float moveRangeX = _playerPos.x - _nowPos.x;
        float moveRangeY = _playerPos.y - _nowPos.y;
        
        //X方向の移動位置決定処理
        if(moveRangeX > 0)
        {
            _xPos = UnityEngine.Random.Range(0, moveRangeX);
        }
        else if(0 > moveRangeX)
        {
            _xPos = UnityEngine.Random.Range(moveRangeX, 0);
        }

        //Y方向の移動位置決定処理
        if (moveRangeY > 0)
        {
            _yPos = UnityEngine.Random.Range(0, moveRangeY);
        }
        else if(0 > moveRangeY)
        {
            _yPos = UnityEngine.Random.Range(moveRangeY, 0);
        }
        
        //目標地点を確定させる
        return Vector3.up * _yPos + Vector3.right * _xPos + transform.position;
    }
    #endregion
}